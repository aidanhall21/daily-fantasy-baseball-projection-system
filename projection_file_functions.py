...
def generate_projection_df_hitters(sals_with_batting_order):
    ...
    sals_with_batting_order['pRBI/PA'] = sals_with_batting_order.apply(lambda row: -0.065 + (1.3 * row['pHR/PA']) + (0.3 * row['leadhittersOBP-HR']) + (0.2 * row['pBA']), axis=1)
    sals_with_batting_order['pR-HR/PA'] = sals_with_batting_order.apply(lambda row: -0.055 + (.245 * row['OBP-HR']) + (0.092 * row['trailhittersOPS']), axis=1)
    ...
    projection_df = sals_with_batting_order[['PlayerID','SlateID', 'Operator', 'OperatorPlayerID', 'TeamID', 'Team', 'OperatorSalary','OperatorGameType', 'SalaryCap', 'OperatorPlayerName', 'OperatorPosition', 'OperatorRosterSlots', 'Team', 'pPA', 'pR', 'pS', 'pD', 'pT', 'pHR', 'pRBI', 'pBB', 'pHP', 'pSB', 'DraftKingsPoints', 'FanDuelPoints']].reset_index(drop=True)
...
def generate_projection_df_pitchers(sals_df, all_starters_dict, FIP_constant):
    ...
    projection_df = sals_df[['PlayerID','SlateID', 'Operator', 'OperatorPlayerID', 'TeamID', 'Team', 'OperatorSalary','OperatorGameType', 'SalaryCap', 'OperatorPlayerName', 'OperatorPosition', 'OperatorRosterSlots', 'Team', 'pIP', 'pW', 'pQS', 'pK', 'pBB', 'pHR', 'pH', 'pHBP', 'pER', 'DraftKingsPoints', 'FanDuelPoints']].reset_index(drop=True)

    return projection_df

def get_classic_slates_by_date(date):
...
def dfs_optimizer(slate, points_and_sal, scoring_type, players_exclude, players_include):

    points_and_sal = points_and_sal[points_and_sal['SlateID']==slate]
    
    #points_and_sal = points_and_sal.loc[:,['PlayerID','OperatorPlayerName', 'OperatorPosition', 'OperatorSalary',scoring_type]]
    ...
    points_and_sal2 = points_and_sal2[~points_and_sal2['OperatorPlayerName'].isin(players_exclude)]
    ...
    values = points_and_sal3[scoring_type].tolist()
    ...
    lineup = pd.concat([lineup, include])

    optimal_players = list(lineup['PlayerID'])
    optimal_player_names = list(lineup['OperatorPlayerName'])
    total_salary_used = lineup['OperatorSalary'].sum()
    projected_points = round(lineup[scoring_type].sum(), 2)

    return optimal_players, optimal_player_names, total_salary_used, projected_points

def get_actual_points_scored_by_lineup(date, player_id_list):
    ...
    total_points = player_points[player_points['PlayerID'].isin(player_id_list)]['FantasyPointsDraftKings'].sum()

    return total_points

def get_best_lineup(date, slate, scoring_type, players_exclude, players_include):
...
def get_sportsdata_projections_by_date(date):
    ...
    player_points = pd.json_normalize(projections)

    points_and_sal = df_player_sal.merge(player_points,on='PlayerID')

    return points_and_sal

def get_sportsdata_optimal(slate, points_and_sal, scoring_type, players_exclude, players_include):

    optimal_players, optimal_player_names, total_salary_used, projected_points = dfs_optimizer(slate, points_and_sal, scoring_type, players_exclude, players_include)
