...
def makePitTable(r):
    for stat in ['AB', 'H', 'D', 'T', 'HR', 'SO', 'BB', 'SF', 'HP', 'CI', 'IPouts', 'R', 'BFP']:
        if stat in r:   pass
        else:   r[stat] = 0
    ab = 0.9*r['IPouts'] + r['H']
    if ab == 0:
    ...
    for b in projectPitchers:
        ...
        for row in pitchers:
            ...
            if row[1] == yr3:
                ...
                y2ip += int(float(row[pitHeaders['IPouts']]))
            elif row[1] == yr1:
                for stat in pitHeaders:
                    try:    playerStat = int(float(row[pitHeaders[stat]]))
                    except: continue
                    components[stat] += 3*playerStat
                y1ip += int(float(row[pitHeaders['IPouts']]))
            else:   continue
        try:
...
##
    header = ['bdbID', 'First', 'Last', 'Year', 'age', 'rel', 'DraftKingsPoints', 'DKp/BF',
              'IP', 'BFP', 'ERA', 'RA',  'W', 'L', 'G',
              'GS', 'CG', 'SHO', 'SV', 'H', 'ER',
...
firstlast = {}
for pl in master:
    firstlast[pl[0]] = [pl[13], pl[14]]

## sample usage 
#marcelPitchingSeason(2012)
# %%
